    ...
    root
      ...
      .map { el ->
        ...
        Leaf(nameLines[0], a.urlFrom()) { doc ->
          ...
          val allNumItems = desc.collectFromLines { it.maybe { quantityFrom() } }
          ...
          ScrapedItem(
            ...
            desc = desc,
            mixed = mixed,
            abv = if (mixed) null else desc.collectFromLines { it.maybe { abvFrom() } }.min(), // Workaround for prose saying "100%"
            available = true,
            offers = setOf(
              Offer(
