...
class RedWillowScraperTest {
  ...
  fun `extracts beer details`() {
    assertEquals(
      ScrapedItem(
        ...
        summary = "Hazy IPA",
        abv = 5.2,
        offers = setOf(Offer(totalPrice = 20.00, quantity = 6)),
        available = true,
  ...
  fun `identifies mixed cases`() {
    ...
    assertFalse(items.any { it.offers.single().quantity == 1 })
  }

  @Test
  fun `falls back to identifying number of items from title`() {
    assertEquals(12, ITEMS.byName("Pale Mixed Case").offers.single().quantity)
  }
